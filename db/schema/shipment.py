...
class Barcode(Base):
    ...
    OrderNum = mcol(VARCHAR(length=256), index=True)
    Barcode = mcol(VARCHAR(length=20), unique=True, nullable=False, index=True)
    MailType = mcol(TEXT)
    MailCat = mcol(TEXT)
    DeclaredValues = mcol(JSONB)
    Dimensions = mcol(JSONB)
    MinDaysShip = mcol(SMALLINT)
    MaxDaysShip = mcol(SMALLINT)
    IndexFrom = mcol(VARCHAR(length=6), nullable=False)
    IndexTo = mcol(VARCHAR(length=6), nullable=False)
    PointFrom = mcol(ForeignKey(ShipPoint.ID, ondelete="CASCADE"), nullable=False)
    PointTo = mcol(ForeignKey(ShipPoint.ID, ondelete="CASCADE"), nullable=False)
    Mass = mcol(INTEGER, nullable=False)
    ShelfLifeDays = mcol(INTEGER, nullable=False)
    InsureValue = mcol(INTEGER)
    CODPayment = mcol(INTEGER)
    TotalRate = mcol(INTEGER)
    TotalRateVat = mcol(INTEGER)
